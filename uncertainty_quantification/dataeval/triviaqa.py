...
def sample_to_prompt(sample, **kwargs):  # used one-shot prompting
    if isinstance(sample['question'], list): # sample['question']이 리스트일 경우
...
@functools.lru_cache()
def get_dataset(tokenizer, split='validation'): # validation triviaqa dataset 
    data = datasets.load_dataset("trivia_qa", "rc.nocontext", split=split) # trivia_qa는 datasets 모듈이 제공하는 데이터셋
    data.save_to_disk("/mnt/aix7101/minsuh-dataset/triviaqa")
    id_mem = set()
    ...
    data = data.map(lambda _: process_data_to_model_inputs(_, tokenizer),
                            ...
                            remove_columns=["search_results", "question_source", "entity_pages"]) # 이런 columns를 제거함
    data.set_format(
        type="torch",
...
if __name__ == '__main__':
    ...
    tokenizer = load_model.load_pretrained_tokenizer('llama-7b-hf')
    #model, tokenizer = models.load_model_and_tokenizer('llama-7b-hf')
    data = get_dataset(tokenizer, split='validation')
    data.save_to_disk("/mnt/aix7101/minsuh-dataset/triviaqa-tokenized")